...
const Dashboard = ({ expanded, setExpanded }) => {
  ...
  const handlePaymentAction = async (paymentData) => {
    try {
      await axios.post('/create-payment', paymentData, { withCredentials: true });
  ...
  const handleExpenseAction = async (expenseData) => {
    try {
      ...
      // Debug log
      console.log('Adding expense with date:', dataToSend.date);
      await axios.post('/create-expense', dataToSend, { withCredentials: true });
  ...
  const handleBudgetGoalAction = async (goalData) => {
    try {
      await axios.post('/create-budget-goal', goalData, { withCredentials: true });
      setToast({ type: 'success', message: 'Budget goal added successfully!' });
      // Optionally refresh stats or goals here
    } catch (error) {
      setToast({
        type: 'error',
